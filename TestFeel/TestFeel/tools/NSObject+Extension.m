...
+(NSArray*)getDeviceSortArr:(NSArray*)deviceArr{
    NSArray *product1 = @[@"Bioneb",@"Aerogo",@"AK-WK-03",@"A15",@"A99",@"A98",@"MiniBear",@"GO3",@"Nanus",@"Air Pro 17",@"A8",@"Air PRO 11",@"AeroBPlus",@"Air Right",@"AirRight",@"Air Pro 007",@"Air Pro 007Y",@"Air Mask",@"AeroB",@"AirICU Max",@"AirHot",@"Air Force",@"AeroCentre",@"AIR INS Model Y",@"AeroIns",@"Air Q+",@"AirUltra 1",@"Air Fly",@"Air Garden",@"Air Kids",@"Air Plus",@"Air Pro IX",@"Air Pro VIII",@"Air ICU Max+",@"Air Plus3",@"AirICU Max+"];
    NSArray *product2 = @[@"Air Smart Extra",@"Air Smart",@"Air Smart T1"];
    NSArray *product3 = @[@"AirFit Smart",@"AirFit 1",@"AirFit 2",@"AirFit 3",@"AirFit 4"];
    NSArray *product4 = @[@"AirBar Pro1",@"AirBar Pro"]; // Ensure product4 is included in sorting

    // Create a dictionary to hold the priority of each element
    NSMutableDictionary *priorityDict = [NSMutableDictionary dictionary];
    for (NSString *product in product1) {
        priorityDict[product] = @(1);
    }
    for (NSString *product in product2) {
        priorityDict[product] = @(2);
    }
    for (NSString *product in product3) {
        priorityDict[product] = @(3);
    }
    for (NSString *product in product4) {
        priorityDict[product] = @(4);
    }

    // Sort the deviceArr based on the priority
    NSArray *sortedDeviceArr = [deviceArr sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
        NSNumber *priority1 = priorityDict[obj1] ?: @(NSIntegerMax);
        NSNumber *priority2 = priorityDict[obj2] ?: @(NSIntegerMax);
        
        if ([priority1 compare:priority2] != NSOrderedSame) {
            return [priority1 compare:priority2];
        }
        
        // If priorities are the same, retain original order
        return [@( [deviceArr indexOfObject:obj1]) compare:@( [deviceArr indexOfObject:obj2])];
    }];
