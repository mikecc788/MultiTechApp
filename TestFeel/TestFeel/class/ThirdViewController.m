...
-(void)setupUI{
    ...
    testBtn1.frame = CGRectMake(210, 350, 50, 36);
    testBtn1.backgroundColor = [UIColor blueColor];
    [testBtn1 setTitle:@"测试1" forState:UIControlStateNormal];
    [testBtn1 addTarget:self action:@selector(testClick1:) forControlEvents:UIControlEventTouchUpInside];

    [self.view addSubview:testBtn1];
}
-(IBAction) testClick1:(id)sender{
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    [[NetworkManager sharedManager]requestWithURL:@"https://jsonplaceholder.typicode.com/photos" parameters:params success:^(id  _Nonnull responseObject) {
